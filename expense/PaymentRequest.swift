...
struct PaymentRequest {
    ...
    static func upload(_ item: Payment, for job: Job) -> Observable<Payment> {
    ...
    static func upload(_ items: [Payment], for job: Job) -> Observable<Void> {
    ...
    static func delete(_ item: Payment) -> Observable<Payment> {
        return ApiProvider.request(Api.deletePayment(id: item.remoteId)).map({ (_) -> Payment in
            return item
        })
    }
    
    static func load(for invoiceId: Int64? = nil, updatedAfter: String?, updateIn context: NSManagedObjectContext) -> Observable<PagedResult<Payment>> {
