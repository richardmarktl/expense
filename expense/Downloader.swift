...
enum DownloaderItem: String, Equatable {
    case none
    case clients
    case items
    case invoices
    ...
    func request(storage: UserDefaults = UserDefaults.standard, context: NSManagedObjectContext) -> Observable<DownloaderItem> {
    ...
    func deleteStorageEntry(storage: UserDefaults = UserDefaults.standard) {
        let storageKey = rawValue + "_last_loaded_date"
    ...
    private func request(_ ctx: NSManagedObjectContext, lastLoadedDate: Date?) -> Observable<Void> {
        ...
        case .invoiceDefaults:
            guard let invoiceDefaults = Defaults.currentDefaults(for: .invoice, in: ctx) else {
                Crashlytics.sharedInstance().recordError("No invoice defaults found")
                return Observable.just(())
            }
        ...
        case .offerDefaults:
            guard let offerDefaults = Defaults.currentDefaults(for: .offer, in: ctx) else {
                Crashlytics.sharedInstance().recordError("No offer defaults found")
                return Observable.just(())
            }
...
class Downloader {
    ...
    open class var instance: Downloader? {
        get {
    ...
    var progressObservable: Observable<DownloaderItem> {
        return progressSubject.asObservable()
    }
    
    private var loadingObs: Observable<Void>?
    
    func restoreFromBackup() -> Observable<Void> {
