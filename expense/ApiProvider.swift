...
extension ApiError: LocalizedError {
    var errorDescription: String? {
        switch self {
        case .invalidJSON:
            return R.string.localizable.invalidJSON()
        case .parameter:
            return R.string.localizable.parameter()
...
struct ApiProvider {
    ...
    static func setupDefaultProvider(_ type: ApiType, online: Observable<Bool> = reachabilityColsure()) {
        switch type {
        case .default:
    ...
    static func request(_ target: Api) -> Observable<Moya.Response> {
        return online
            .do(onNext: { isOnline in
                logger.verbose("Are we online for the request: \(isOnline)")
                if !isOnline {
                    #if !IS_EXTENSION
                    Crashlytics.sharedInstance().recordError("Device Offline")
                    #endif
                }
            })
            .filter { $0 == true }
