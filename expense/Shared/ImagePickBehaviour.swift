...
class ImagePickBehaviour {
    ...
    fileprivate func checkPermissionAndPresentImagePicker(with pickerType: UIImagePickerController.SourceType) {

        let check = ImagePickerPermissionCheck(controller: rootController)
    ...
    fileprivate func presentImagePicker(with pickerType: UIImagePickerController.SourceType) {
        bag = DisposeBag()
        ...
        _ = UIImagePickerController.rx.createWithParent(rootController, animated: true) { (picker) in
                ...
                .map { (info: [UIImagePickerController.InfoKey: Any]) -> UIImage in
                    ...
                    guard let croppedImage = info[UIImagePickerController.InfoKey.editedImage] as? UIImage else {
                        return image
                    }
