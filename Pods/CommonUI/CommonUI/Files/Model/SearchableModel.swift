...
open class SearchableModel<ItemType: Filterable, T>: Model<T> {
    /// Model that combines the data loading + searching
    ///
    /// - Parameters:
    ///   - searchObservable: observable which changes when the search string changes
    ///   - loadObservable: data load observable
    public init(searchObservable: Observable<String>, loadObservable: Observable<[ItemType]>,
                itemMapper: @escaping (([ItemType]) -> Section<T>),
                defaultSections: [Section<T>], with context: NSManagedObjectContext) {
