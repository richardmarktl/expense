...
public struct JobLocalizationRequest {
    
    public static func upload(_ localization: JobLocalization) -> Observable<JobLocalization> {
    ...
    public static func delete(_ item: JobLocalization) -> Observable<JobLocalization> {
        return ApiProvider.request(Api.deleteLocalization(id: item.remoteId)).map { _ -> JobLocalization in
            return item
        }
    }
    
    public static func load(updatedAfter: String?, updateIn context: NSManagedObjectContext) -> Observable<PagedResult<JobLocalization>> {
