...
public enum InvoiceRequestError: Error {
    case next
}
...
public struct InvoiceRequest {
    ...
    public static func delete(_ item: Invoice) -> Observable<Invoice> {
        return ApiProvider.request(Api.deleteInvoice(id: item.remoteId)).map { _ -> Invoice in
            return item
        }
    }
    
    public static func uploadSignature(_ item: Invoice) -> Observable<Invoice> {
        ...
        return ApiProvider.request(Api.nextInvoiceId)
            ...
            .map({ (json: Any) -> String in
                guard let json = json as? JSONDictionary, let number = json["number"] as? String else {
                    throw InvoiceRequestError.next
                }
                return number
            })
    }
    ...
    public static func load(updatedAfter: String?, updateIn context: NSManagedObjectContext) -> Observable<PagedResult<Invoice>> {
    ...
    public static func load(with remoteId: Int64, context: NSManagedObjectContext) -> Observable<Invoice> {
