...
public enum RecipientState: Int32 {
    case notSend = 0
    case sent = 1
    case deferral = 2
    case bounced = 3
...
extension Recipient: Fetchable {
    ...
    public var sort: Int16 {
        get {
            return 0
        }
    }

    public var typedState: RecipientState {
    ...
    public var signatureImagePath: String? {
        if let uuid = uuid {
            return "cs_" + uuid
        }
        return nil
    ...
    public var signatureNameAndDate: String {
    ...
    public func loadRecipientSignature() -> Observable<[String: Any]>? {
        ...
        if ImageStorage.hasItemStoredOnFileSystem(in: FileSystemDirectory.imageAttachments, filename: localPath) {
            observable = ImageStorage.loadImage(in: FileSystemDirectory.imageAttachments, for: localPath)
        } else {
            observable = ImageStorage.download(fromURL: signature, filename: localPath, storeIn: FileSystemDirectory.imageAttachments)
        }
