...
typealias MailParameter = (data: Data, text: String, recipients: [[String: String]])
...
extension Api: TargetType {
    ...
    var sampleData: Data {
        ...
        case .listClients(let cursor):
            return cursor == nil ? stubbedResponse("clients") : stubbedResponse("clients_abc")
        case .createClient:
            return stubbedResponse("client")
        case .updateClient:
            return stubbedResponse("client_updated")
        case .listItems(let cursor):
            return cursor == nil ? stubbedResponse("items") : stubbedResponse("items_abc")
        case .createItem:
    ...
    var task: Task {
        ...
        case .updateLogoForAccount(let data, let filename):
            ...
            let logo = MultipartFormData(provider: .data(data), name: "logo", fileName: filename?.withJPGFileExtension, mimeType: AnnotationMimeType.jpeg.rawValue)
            return Task.uploadMultipart([logo])
            
        case .updateRecipientSignature( _, _,let data, let filename):
            let file = MultipartFormData(provider: .data(data), name: "signature", fileName: filename.withJPGFileExtension, mimeType: AnnotationMimeType.jpeg.rawValue)
    ...
    var headers: [String: String]? {
        ...
        default:
            ...
            headerFields["X-Client-Version"] = Bundle.main.appVersion
