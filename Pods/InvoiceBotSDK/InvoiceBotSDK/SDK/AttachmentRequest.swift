...
public struct AttachmentRequest {
    ...
    public static func upload(_ item: Attachment, for job: Job) -> Observable<Attachment> {
    ...
    public static func upload(_ items: [Attachment], for job: Job) -> Observable<Void> {
    ...
    public static func delete(_ item: Attachment) -> Observable<Attachment> {
        ...
        return ApiProvider.request(Api.deleteAttachment(path: path, id: item.remoteId)).map({ (_) -> Attachment in
            if let uuid = item.uuid {
                ImageStorage.deleteImage(in: FileSystemDirectory.imageAttachments, for: uuid)
            }
            return item
        })
    }
    
    public static func uploadAttachmentContent(_ item: Attachment) -> Observable<Attachment> {
        guard
            let job = item.job,
            let uuid = item.uuid,
    ...
    private static func create(with uuid: String, filename: String, remoteId: Int64, sort: Int16, path: Path, attachmentRemoteId: Int64?) -> Observable<Moya.Response> {
        ...
        return ImageStorage.loadImage(in: FileSystemDirectory.imageAttachments, for: uuid).observeOn(background).map({ (storageItem) -> Data? in
    ...
    public static func load(for path: Path, updatedAfter: String?, updateIn context: NSManagedObjectContext) -> Observable<PagedResult<Attachment>> {
