...
extension JobDesign: Fetchable, Createable {
    
    public typealias FetchableType = JobDesign
    public typealias CreatedType = JobDesign
    ...
    public static func create(in context: NSManagedObjectContext) -> JobDesign {
    ...
    public static func current(in context: NSManagedObjectContext) -> JobDesign {
        return JobDesign.allObjects(fetchLimit: 1, context: context).first!
    }
    
    public static func migrateFromAccountIfNeeded(account: Account, in context: NSManagedObjectContext) -> Observable<JobDesign> {
