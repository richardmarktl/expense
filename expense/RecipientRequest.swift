...
struct RecipientRequest {
    ...
    static func delete(_ recipients: [Recipient]) -> Observable<Void> {
    ...
    /// - Returns: an observable
    static func delete(_ recipient: Recipient) -> Observable<Recipient> {
        ...
        return ApiProvider.request(Api.deleteRecipient(path: path, id: recipient.remoteId)).map({ (_) -> Recipient in
            if let name = recipient.signatureImagePath {
                ImageStorage.deleteImage(for: name)
            }
            return recipient
        })
    }
    
    static func uploadSignature(for recipient: Recipient) -> Observable<Recipient> {
        ...
        return ImageStorage.loadImage(for: filePath).observeOn(background).map({ (storageItem) -> Data? in
    ...
    static func load(for path: Path, jobId: Int64? = nil, updateIn context: NSManagedObjectContext) -> Observable<PagedResult<Recipient>> {
