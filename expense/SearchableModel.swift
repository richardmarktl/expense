...
class SearchableTableModel<ItemType: Filterable>: TableModel {
    ...
    init(searchObservable: Observable<String>, loadObservable: Observable<[ItemType]>, itemMapper: @escaping (([ItemType]) -> TableSection),
         defaultSections: [TableSection], with context: NSManagedObjectContext) {
