...
class DiscountCell: ReusableCell, ConfigurableCell, InputAccessoryAble {
    typealias ConfigType = DiscountEntry
    ...
    func configure(with item: DiscountEntry) {
        ...
        let discountSwitcher = DiscountSwitcher(frame: CGRect.zero)
        discountSwitcher.isAbsoluteValue = item.data.value.isAbsolute
        discountSwitcher.isAbsoluteValueObservable.subscribe(onNext: { [weak self](isAbsolute) in
            
            item.update(with: isAbsolute)
            self?.titleLabel?.text = item.title
            
        }).disposed(by: reusableBag)
        
        registerAccessory(for: textField, customCenterView: discountSwitcher)
    }
}
