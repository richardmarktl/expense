...
public struct PagedResult<T: BaseItem> {
    let results: [T]
    let nextPageCursor: String?
}

public func updateObjectWithJSON<T: BaseItem>(_ object: T, considerLocalModifiedTimestamp: Bool = false) -> (Any) -> T {
...
public func updateRemoteItemWithJSON<T: BaseItem & Fetchable>(_ remoteId: Int64, in context: NSManagedObjectContext, considerLocalModifiedTimestamp: Bool = false,
...
public func updateObjectsFromJSON<T: BaseItem>(_ context: NSManagedObjectContext, considerLocalModifiedTimestamp: Bool = false,
...
public func updateObjectsFromJSONIngorePagination<T: BaseItem>(_ context: NSManagedObjectContext, considerLocalModifiedTimestamp: Bool = false) -> (Any) -> [T] {
    return { JSON -> [T] in
        return updateObjectsFromJSON(context, considerLocalModifiedTimestamp: considerLocalModifiedTimestamp)(JSON).results
    }
}

public func extractNextPageFromJSON(_ JSON: JSONDictionary) -> String? {
