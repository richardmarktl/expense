...
extension Job {
    ...
    var changedAttachements: [Attachment] {
        guard let ctx = managedObjectContext else {
            return []
        }
    ...
    var attachmentTyped: [Attachment] {
        return attachments?.allObjects as? [Attachment] ?? []
    }
    
    var recipientsTyped: [Recipient] {
    ...
    var referenceRelationId: String {
    ...
    func rxChangesOnSelf(in context: NSManagedObjectContext) -> Observable<Job> {
        if self is Invoice {
    ...
    func loadUserSignature() -> Observable<ImageStorageItem> {
        ...
        if ImageStorage.hasItemStoredOnFileSystem(filename: localSignatureName) {
            return ImageStorage.loadImage(for: localSignatureName)
        } else if let signature = signature {
            return ImageStorage.download(fromURL: signature, filename: localSignatureName)
        }
    ...
    func needsSignatureUpdate() -> Observable<(Data?, FileUpdate)> {
        if signedOn != nil {
            // no signature set, so we need an upload
            if signature == nil, let path = signatureImageName {
                return ImageStorage.loadImage(for: path).map({ (storageItem) -> (Data?, FileUpdate) in
                    return (UIImagePNGRepresentation(storageItem.image), .update)
                })
            }
    ...
    /// - Returns: a Boolean
    func hasChanges(ignore: [String]) -> Bool {
    ...
    func markAsSend(_ sendTo: [String] = []) {
