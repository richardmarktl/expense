...
extension Attachment: Fetchable {
    ...
    public func update(from storageItem: ImageStorageItem) {
        uuid = storageItem.filename
        path = storageItem.imagePath
        thumbPath = storageItem.thumbnailPath
    }
    
    public static func create(in context: NSManagedObjectContext) -> Attachment {
    ...
    public static func create(from attachment: Attachment, in context: NSManagedObjectContext) -> Attachment {
        ...
        if let oldFilename = attachment.uuid {
            let paths = ImageStorage.duplicate(in: FileSystemDirectory.imageAttachments, for: oldFilename, newFilename: filename)
